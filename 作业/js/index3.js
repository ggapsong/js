...
button.onclick = function () {
    ...
    if (first == "" && second == "" && third == "") {
        ...
        add();
        all.querySelector('input').checked = '';
    }
    ...
    function add() {
        ...
        imgs[2].onclick = function () {
            ...
            let same = document.querySelectorAll("li input");
            if (same.length == 0) {
                document.querySelector(".all input").checked = false;
            }
            same.forEach((item) => {
        ...
        //全选全不选
        let checkboxall = document.querySelector(".all input"); //全选
        let checkeds = Array.from(document.querySelectorAll("li input")); //所有的input
        checkboxall.onchange = function () {
            Array.from(document.querySelectorAll("li input")).forEach((item) => {
                item.checked = checkboxall.checked;
            })
        }
        checkeds.forEach((item) => {
            item.onchange = function () {
                let isCheckAll = Array.from(document.querySelectorAll("li input")).every((item) => {
                    return item.checked;
                })
